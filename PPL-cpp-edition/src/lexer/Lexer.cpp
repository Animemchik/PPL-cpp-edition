...
void Lexer::tokenizeChar() {
	...
	if (current == '\\') {
		...
		switch (current) {
			...
			case '\'': {
				errors.push_back(new InvalidCharLiteralE(code, filename, row, col));
				return;
			}
			default: {
				errors.push_back(new InvalidCharLiteralE(code, filename, row, col));
		...
		errors.push_back(new CharNotCompleteE(code, filename, row, col));
		return;
	} else if (current == '\'') {
		errors.push_back(new EmptyCharE(code, filename, row, col));
		return;
	} else {
		char value = current;
		current = next();
